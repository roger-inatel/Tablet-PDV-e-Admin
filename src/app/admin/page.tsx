...
export default function DashboardPage() {
  ...
  const data = useMemo(() => {
    const occupied = occupiedTables(tables);
    const livres = freeTables(tables);
    const comandas = openComandas(tables);
    const active = activeWaiters(waiters);
    const wById = waitersById(waiters);
    ...
    const kpis = [
      ...
      {
        label: "Garçons em serviço",
        value: `${active.length}/${waiters.length}`,
        helper: "turno atual",
      },
      {
        label: "Em aberto",
        value: fmt(openTotal(tables)),
        helper: "a receber nas mesas",
      },
    ];

    const comandaRows = comandas.map((t) => {
      const w = wById[t.waiterId ?? ""];
      const status = comandaRowStatus(t);
      return {
        id: t.id,
        mesa: `Mesa ${t.num}`,
        waiter: w ? firstName(w.name) : "—",
        initials: w ? w.initials : "--",
        color: w ? w.color : "#94a3b8",
        items: t.items.reduce((a, i) => a + i.qty, 0),
        total: fmt(total(t.items)),
        status,
      };
    });
    ...
    return { kpis, comandaRows, bars, garcons, comandasCount: comandas.length };
  }, [tables, waiters]);
            ...
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1.7fr)_minmax(280px,1fr)]">
              ...
              <div className="rounded-card border border-line bg-white p-5">
                <div className="mb-3 flex items-center justify-between">
                  <h2 className="m-0 text-[1.08rem] text-navy">Comandas abertas agora</h2>
                  <span className="text-[0.82rem] text-ink-muted">
                    {data.comandasCount} mesas em atendimento
                  </span>
                </div>
                {/* Mobile: cards */}
                <div className="grid gap-2.5 md:hidden">
                  {data.comandaRows.map((r) => (
                    <div
                      key={r.id}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <StatusChip kind={r.status.kind}>{r.status.label}</StatusChip>
                      </div>
                    ...
                    {data.garcons.map((g) => (
                      ...
                      >
                        ...
                        <div className="grid min-w-0 flex-1 gap-px">
                          ...
                          <span className="text-[0.76rem] text-ink-muted">{g.role}</span>
                        </div>
