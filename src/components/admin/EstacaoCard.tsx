...
interface EstacaoCardProps {
  estacao: EstacaoConfig;
  /** Pedidos currently in this station's queue (not fully ready). */
  filaAtiva: number;
}

/** Station (KDS) card — replaces the old printer card. */
export function EstacaoCard({ estacao, filaAtiva }: EstacaoCardProps) {
  return (
      ...
      <div className="flex items-start justify-between gap-2.5">
        <div className="flex items-center gap-2.5">
          <span
            ...
            style={{ background: estacao.cor }}
          >
            <Icon name={estacao.icone} size={18} />
          </span>
          <div>
            <strong className="block text-[1rem] text-navy">{estacao.nome}</strong>
            <span className="text-[0.8rem] text-ink-muted">{estacao.descricao}</span>
          </div>
        </div>
        <StatusChip kind={filaAtiva > 0 ? "blue" : "green"}>
          {filaAtiva > 0 ? `${filaAtiva} na fila` : "Sem fila"}
        </StatusChip>
      </div>
